...
class AdditiveGP:
    ...
    def __init__(
        ...
        config: AdditiveGPConfig | None = None,
    ) -> None:
        ...
        self.groups = self._normalize_groups(
            groups,
            self.X_raw.shape[1]
        )
    ...
    def _fit_models(self) -> None:
        ...
        # 当前为简单 additive 实现。
        # 单 group 时 target_share == self.Y。
        # 多 group 的责任分配后续根据 MOSTAR 重新实现。
        target_share = self.Y / max(len(self.groups), 1)
    ...
    def predict_scaled(
        ...
        X_scaled = self.x_scaler.transform(X_new_2d)

        mean = np.zeros(
            (X_scaled.shape[0], self.output_dim),
            dtype=float,
        )
        ...
        for group, model in zip(
            ...
            sub_mean, sub_var = model.predict(
                X_scaled[:, group]
            )
    ...
    def forward(
        ...
        variance = (
            variance_scaled
            * np.square(self.y_scaler.scale_)
        )
